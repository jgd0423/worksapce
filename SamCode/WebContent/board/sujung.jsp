...
<table border="1" align="center" width="80%">
	...
	<tr>
		...
		<td>
			<textarea name="content" id="content" style="width:300px; height:100px;" wrap="hard">${dto.content }</textarea>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<c:if test="${dto.noticeNo > 0}">
				<input type="checkbox" name="noticeGubunCheckBox" value="T" onclick="clickChk('noticeGubun');" checked> 공지글 체크
			</c:if>
			<c:if test="${dto.noticeNo == 0}">
	...
	<tr>
		...
		<td>
			...
			<c:if test="${dto.secretGubun == 'T' }">
				<input type="checkbox" name="secretGubunCheckBox" value="T" onclick="clickChk('secretGubun');" checked> 비밀글 체크
			</c:if>
			<c:if test="${dto.secretGubun != 'T' }">
...
<script>
	$(document).ready(function(){
		...
		$("#btnSujung").click(function(){
			if (confirm('수정하시겠습니까?')) {
				suntaek_proc('sujungProc', '0', $("#span_no").text());
			}
		});
